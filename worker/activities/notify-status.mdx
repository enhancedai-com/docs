...
|-----------|------|-------------|
| `nodes` | `Record<string, { status: string }>` | Object mapping node IDs to their status objects. Each status object contains a `status` field (`PENDING`, `COMPLETED`, `FAILED`, or `CANCELLED`) |
