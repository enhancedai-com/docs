...
- **Zustand store** — `storeVersionControl` holds `isProductionView`, whether the user is viewing the deployed (production) version
- **UI components** — `VersionControlDropdown`, `RestoreConfirmModal`, and `VersionNameModal`

### Data flow

1. `AgentTopBar` calls the `useAgentVersionControl(params.flowId)` hook directly
2. The hook fetches versions, deployed version, and history using React Query hooks
3. It computes `VersionControlProps` (version items, handlers, UI state) in a `useMemo` and returns it as a plain value
4. `AgentTopBar` renders `VersionControlDropdown` inside a popover when `versionControlProps` is defined
5. User actions in the dropdown call handlers returned by `useAgentVersionControl`, which trigger mutations and page reloads
...
`storeVersionControl` only tracks whether the user is viewing the deployed (production) version. It does not hold `VersionControlProps` — those are computed and returned directly by the `useAgentVersionControl` hook (see [Data flow](#data-flow)), not shared through a store.
...
- `VersionControlProps` is `undefined` when version control is disabled. The dropdown only renders when `useAgentVersionControl` returns a defined value.
