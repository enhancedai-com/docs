...
```
commandTextNode       scripting              apiCaller
mathFunction           counterNode            outputController
pullData               pushData               mcpNode
imageGenerator         commandContentNode     voiceBoxNode
webSearch              webCrawling            audioReaderNode
sqlQuerier
```
...
```
thirdPartyIntegration    libraryNode    reportBuilder    nodesBox
```
...
Large `outputData` payloads (above `CLAIM_CHECK_THRESHOLD_BYTES`) are offloaded to S3 and stored as a `{ __claimRef: <key>, bytes, preview }` envelope. `NodeExecutionsService.findLatestByExecAndNode` transparently resolves the claim ref before returning. Redeeming one of those keys is authorized against the run that produced it — see [Claim-check authorization](/main-api/specific-features/claim-check-authorization).
